...
def pzmap(sys_list, sys_names=None, plot=True, title="Pole-Zero Map", fig=None):
    """
    Plot a pole/zero map for a linear system.
    ...
    sys_names: list of strings, optional
        List of the names of the systems to plot.
    plot: bool, optional
        If ``True`` a graph is generated with Matplotlib,
        otherwise the poles and zeros are only computed and returned.
    ...
    if fig is None:
        ...
        fig.layout = go.Layout(
            title=title,
            xaxis=dict(title="Real Axis (1/seconds)"),
            yaxis=dict(title="Imaginary Axis (1/seconds)"),
    ...
    for sys_index, sys in enumerate(sys_list):
        ...
        if sys_names is None:
            sys_name = "sys{}".format(sys_index+1)
        else:
        ...
        if len(poles) > 0:
            trace_poles = go.Scatter(
                ...
                mode="markers",
                marker=dict(
                    color=color_list_std[sys_index],
                    size=10,
                    symbol="x-thin",
                    line_width=2,
                    line_color=color_list_std[sys_index],
                ),
                name="Poles",
                legendgroup=sys_name,
                legendgrouptitle_text=sys_name,
            )
    ...
    for sys_index, sys in enumerate(sys_list):
        ...
        if len(poles) > 0:
            trace_poles = go.Scatter(
                ...
                mode="markers",
                marker=dict(
                    color=color_list_std[sys_index], size=10, symbol="x-thin", line_width=2, line_color=color_list_std[sys_index]
                ),
                name="Poles",
...
# %% Old function step (to use with Matplotlib)
def step_(sys, T=None, resolution=10000, NameOfFigure="", sysName="", linestyle="-"):
    ...
    return [yout, t]
# %% Function to plot the step results on an interactive graph (=> JupyterBook)
# Plot DC gain
def plot_DCgain(
    t,
    yout,
    ...
    y: 1D array
        Response of the system.
    figure: plotly.graph_objs.FigureWidget
        Figure where the step response is plotted.
    sys_name: String
        Name of the system.
    color : str, optional
        Color of the plotted points, by default "black"
    legendgroup : str, optional
        The legend group in which the plot belongs, by default "DC gain"
    showlegend : bool, optional
        To show the plot name in the legend, by default True
    visible : str, optional
        To show directly the plot on the figure, by default "legendonly"
    """
    ...
    figure.add_trace(
        go.Scatter(
            ...
            name="{} - DCGain".format(sys_name),
            legendgroup=legendgroup,
            hoverinfo="none",
            customdata=[info.SettlingTime],
            hovertemplate="<b>DC Gain</b>: %{y:.3f}<br>",
...
def plot_overshoot(
    ...
    # Calculate the overflow by taking the max value returned by step and dividing 
    # it by the final value
    # ----------------------------------------------------------------------------
    osIndice = np.where(yout == np.amax(yout))  # returns an array tuple
    osIndice = osIndice[-1][-1]  # reads the last index meeting the condition
    ...
    figure.add_trace(
        go.Scatter(
            x=[t[osIndice], t[osIndice]],
            y=[0, yout[osIndice]],
            mode="lines",
            line=dict(color=color, width=2, dash="dot"),
            name=f"{sys_name} - Overshoot vertical line",
            legendgroup=legendgroup,
            showlegend=False,
            visible=visible,
        )
    ...
    figure.add_trace(
        go.Scatter(
            x=[t[0], t[osIndice]],
            y=[yout[osIndice], yout[osIndice]],
            mode="lines",
            line=dict(color=color, width=2, dash="dot"),
            name=f"{sys_name} - Overshoot horizontal line",
            legendgroup=legendgroup,
            showlegend=False,
            visible=visible,
        )
    ...
    figure.add_trace(
        go.Scatter(
            ...
            name=f"{sys_name} - Overshoot",
            legendgroup=legendgroup,
            hoverinfo="none",
            customdata=[info.Overshoot],
            hovertemplate="<b>Peak</b>: %{y:.3f}<br>" +
                          "<b>Peak time</b>: %{x:.3f}s<br>" +
                          "<b>Overshoot</b>: %{customdata:.3f}%<br>",
            showlegend=showlegend,
            visible=visible,
        ),
    )
...
def plot_rise_time(
    ...
    info,
    sys_name,
    RiseTimeThresholdMin=0.1,
    RiseTimeThresholdMax=0.9,
    color="black",
    legendgroup="Rise time",
    showlegend=True,
    visible = "legendonly"
    ):
    ...
    sys_name: String
        Name of the system.
    RiseTimeThresholdMin : float, optional
        Defines the lower threshold for RiseTime computation, by default 0.1.
    RiseTimeThresholdMax : float, optional
        Defines the upper threshold for RiseTime computation, by default 0.9.
    color : str, optional
        Color of the plotted points, by default "black"
    legendgroup : str, optional
        The legend group in which the plot belongs, by default "DC gain"
    ...
    # Calculation of the rise time as a function of the treshold (default: from 
    ...
    figure.add_trace(
        go.Scatter(
            x=[t[RiseTimeThresholdMinIndice], t[RiseTimeThresholdMinIndice]],
            y=[0, yout[RiseTimeThresholdMaxIndice]],
            mode="lines",
            line=dict(color=color, width=2, dash="dot"),
            name=f"{sys_name} - RiseTime left limit",
            legendgroup=legendgroup,
            showlegend=False,
            visible=visible,
        )
    ...
    figure.add_trace(
        go.Scatter(
            x=[t[RiseTimeThresholdMaxIndice], t[RiseTimeThresholdMaxIndice]],
            y=[0, yout[RiseTimeThresholdMaxIndice]],
            mode="lines",
            line=dict(color=color, width=2, dash="dot"),
            name=f"{sys_name} - RiseTime right limit",
            legendgroup=legendgroup,
            showlegend=False,
            visible=visible,
        )
    ...
    figure.add_trace(
        go.Scatter(
            x=[t[0], t[RiseTimeThresholdMaxIndice]],
            y=[yout[RiseTimeThresholdMaxIndice], yout[RiseTimeThresholdMaxIndice]],
            mode="lines",
            line=dict(color=color, width=2, dash="dot"),
            name=f"{sys_name} - RiseTime horizontal limit",
            legendgroup=legendgroup,
            showlegend=False,
            visible=visible,
        )
    ...
    figure.add_trace(
        go.Scatter(
            x=[t[RiseTimeThresholdMaxIndice]],
            y=[yout[RiseTimeThresholdMaxIndice]],
            mode="markers",
            marker=dict(
                color=color,
                size=6,
            ...
            name=f"{sys_name} - RiseTime",
            legendgroup=legendgroup,
            hoverinfo="none",
            customdata=[info.SettlingTime],
            hovertemplate="<b>RiseTime</b>: %{x:.3f}s<br>",
            showlegend=showlegend,
            visible=visible,
        ),
    )
...
def plot_settling_time(
    ...
    sys_name: String
        Name of the system.
    SettlingTimeThreshold : float, optional
        Threshold of the settling time, by default 0.05
    color : str, optional
        Color of the plotted points, by default "black"
    legendgroup : str, optional
        The legend group in which the plot belongs, by default "DC gain"
    ...
    # Response time calculation at x% (5% by default)
    # -----------------------------------------------
    delta_values = yout[-1] - yout[0]
    ...
    figure.add_trace(
        go.Scatter(
            x=[0, max(t)],
            y=[yout[0] + delta_values * (1 + SettlingTimeThreshold), yout[0] + delta_values * (1 + SettlingTimeThreshold)],
            mode="lines",
            line=dict(color=color, width=2, dash="dot"),
            name="{} - Up limit".format(sys_name),
            legendgroup=legendgroup,
    ...
    figure.add_trace(
        go.Scatter(
            x=[0, max(t)],
            y=[
                yout[0] + delta_values * (1 - SettlingTimeThreshold), yout[0] + delta_values * (1 - SettlingTimeThreshold)],
            mode="lines",
            line=dict(color=color, width=2, dash="dot"),
            name="{} - Low limit".format(sys_name),
            legendgroup=legendgroup,
            showlegend=False,
            visible=visible,
        )
    ...
    figure.add_trace(
        go.Scatter(
            x=[t[settlingTimeIndice]],
            y=[yout[settlingTimeIndice]],
            mode="markers",
            marker=dict(
                color=color,
                size=6,
            ...
            name=f"{sys_name} - SettlingTime",
            legendgroup=legendgroup,
            hoverinfo="none",
            customdata=[info.SettlingTime],
            hovertemplate="<b>SettlingTime</b>: %{x:.3f}s<br>",
            showlegend=showlegend,
            visible=visible,
        ),
    )
    ...
    figure.add_trace(
        go.Scatter(
            x=[t[settlingTimeIndice], t[settlingTimeIndice]],
            y=[0, yout[settlingTimeIndice]],
            mode="lines",
            line=dict(color=color, width=2, dash="dot"),
            name=f"{sys_name} - SettlingTime",
            legendgroup=legendgroup,
...
# Function to plot and get informations of a step response
def step_info(
    t,
    yout,
    figure,
    ...
    color: String, optional
        Color of the plotted points (default is "black").
    ...
    plot_os: bool, optionnal
        Plot the overshoot point if True (default is True).

    DCGain_plot: bool, optionnal
        Plot the DC gain point if True (default is True).
    ...
    # -----------
    if DCGain_plot:
        plot_DCgain(t, yout, figure, info, sys_name, color=color, legendgroup=f"{sys_name} - Characteristics",showlegend=False)
    
    # Settling time
    # -------------
    if plot_st:
        plot_settling_time(t, yout, figure, info, sys_name, SettlingTimeThreshold=SettlingTimeThreshold,color=color, legendgroup=f"{sys_name} - Characteristics",showlegend=False)
    
    # Rise time
    # ---------
    if plot_rt:
        plot_rise_time(t, yout, figure, info, sys_name, RiseTimeThresholdMin=RiseTimeThresholdMin, RiseTimeThresholdMax=RiseTimeThresholdMax,color=color, legendgroup=f"{sys_name} - Characteristics",showlegend=False)
    
    # Overshoot
    # ---------
    if plot_os:
        plot_overshoot(t, yout, figure, info, sys_name, color=color, legendgroup=f"{sys_name} - Characteristics",showlegend=False)

    # Show the legend
    # ---------------
    # Add an invisible point in the middle of the trace to be able to toggle all the characteristics plots via the legend
    middelIndex = (len(t))//2   # // to get an integer
    figure.add_trace(go.Scatter(
        x=[t[middelIndex]],
        y=[yout[middelIndex]],
        legendgroup=f"{sys_name} - Characteristics",
        name=f"{sys_name} - Characteristics",
        mode="markers",
        marker=dict(
                color=color,
                size=1,
...
def step(
    sys_list, sys_names=None, T=None, resolution=10000, fig=None, title="Step response", data=False, plot_infos=False, givenColorList=None
):
    """
    Trace the step with the given parameters.
    ...
    sys_names: String or list of Strings, optional
        Name of the systems to plot (default is None).
    ...
    data: bool, optionnal
        Return the plot data (yout, t and info object) for all the systems plotted.
    
    plot_infos: bool, optionnal
        Plot the informations about the step response.

    givenColorList: list of Strings, optional
        List of colors to use for the traces (default is None).
    ...
    info: Info object
        Informations about the step response.
    """

    # Define the color list
    if givenColorList is None:
        color_list = color_list_std
    else:
        color_list = givenColorList
    ...
    for sys_index, sys in enumerate(sys_list):
        ...
        if(sys_names is None):
            ...
            trace_step = go.Scatter(
                x=t,
                y=yout,
                name=sys_names[sys_index],
                line=dict(color=color_list[sys_index]),
            )
        fig.add_trace(trace_step)

        if plot_infos:
            info=step_info(t, yout, fig, f"sys{sys_index+1}", color=color_list[sys_index])
...
def step2(
    ...
    for sys_index, sys in enumerate(sys_list):
        ...
        trace_step = go.Scatter(
            ...
            y=yout,
            name="sys{}".format(sys_index + 1),
            line=dict(color=color_list_std[sys_index]),
        )
        fig.add_trace(trace_step)
...
# %% Old function step_info (to use with Matplotlib)
def step_info_(
    t,
    yout,
    info=None,
...
# %% Plot the speed error of a system in response to a unitary ramp
def plot_speed_error(sys):
    """Plot the speed error of a system in response to a unitary ramp
    ...
    -------
    fig: plotly.graph_objs.FigureWidget
        Figure where the response to a unitary ramp is plotted.
    ev: float
        The speed error of the system in response to a unitary ramp
    """
    ...
    y, t, __ = ml.lsim(sys,s,t)

    # Affichage des graphes
    # Reference ramp
    trace_speedResponse = go.Scatter(
        x=t,
        y=s,
        name="Input ramp",
        visible = "legendonly"
    )
    fig.add_trace(trace_speedResponse)

    # Response to the ramp
    trace_rampReference = go.Scatter(
        x=t,
        y=y,
        name="System response",
        visible = "legendonly"
    )
    fig.add_trace(trace_rampReference)

    # Speed error
    ev = s[-1]-y[-1]
    trace_speedError= go.Scatter(
        x=t,
        y=s-y,
        name="Speed error"
    )
    fig.add_trace(trace_speedError)
...
def nichols(sys_list, sys_names=None, show_grid=True, margins=False, givenColorList=None):
    """Nichols plot for a system
    ...
    Args:
        sys_list (system or list of systems): A single system or a list of systems to analyse
        sys_names (list of strings, optional): The names of the systems to plot. Defaults to None.
        ...
        givenColorList (list of strings, optional): The colors to use for the different systems. Defaults to None.
    ...
    # To be able to toggle the visibility of the grid via the legend and to changes the default names
    for trace_index, trace in enumerate(fig.data):
        # If the trace is a system
        if trace_index < len(sys_list):
            fig.data[trace_index].line.update(color=color_list[trace_index])
            if sys_names is not None:
                fig.data[trace_index].update(name=sys_names[trace_index])
        # If the trace is the grid
        elif trace_index > len(sys_list)-1 and trace_index != len(fig.data)-1:
        ...
        if margins:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="lines",
                    line=dict(color="black", width=2, dash="solid"),
                    name="Margins - {}".format(sys_name),
                    legendgroup="Margins - {}".format(sys_name),
                    showlegend=False,
                    visible=visible,
                )
            )
...
def nyquist(sys_list):
    ...
    fig.update_layout(
        title="Nyquist plot", template="plotly_white", height=800, width=800
    )
